...
class sitemin_installer_install {
    var $access_file = [ /*_X_INSTALL_FILE0, no to test*/
    _X_INSTALL_FILE1, _X_INSTALL_FILE2, _X_INSTALL_FILE3, _X_INSTALL_FILE4, _X_INSTALL_FILE5, ];
    ...
    function test_connection($q) {
        $q = xpAS::escape($q);
        $c['db'] = ['user' => $q['user'], 'host' => $q['host'], 'password' => $q['password'], 'database' => addslashes($q['database']), ];
        sleep(1);
        try {
            $db = xpPdo::conn($c);
            return false;
    ...
    function install($q) {
        ...
        */
        ...
        file_put_contents(str_replace('.sample', '', _X_INSTALL_FILE5), $con);

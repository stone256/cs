...
class sitemin_api_model_acl {
    ...
    function gets($q) {
        ...
        if ($q['sort']) $order = "ORDER BY " . ($q['sort'] {
            0
        } == '-' ? substr($q['sort'], 1) . " DESC " : $q['sort']);
        //get counter
        $sql = "
			SELECT count(*) as c
			FROM api_user as u
    ...
    function saves($q) {
        ...
        foreach ($gateway as $g) {
            //$g = '/api'.$g;
            if ($acl = _factory('sitemin_api_model_api')->get_acl_by_url($g)) xpTable::load($this->table)->insert(array('api_user_id' => $user, 'api_id' => $acl['id']));
        }
    }
    /**

...
class module_model {
    //check module
    function test_permission($files) {
        if (!is_array($files)) $files = array($files);
        foreach ($files as $f) {
            $p[is_writable($f) ? 'ok' : 'failed'][] = _rp($f);
        }
        return $p;
    }
    ...
    function create($q) {
        ...
        $p = _X_MODULE . "/{$name}";
        if (!mkdir($p, 0775, 1)) $msh[] = 'module create failed';
        ...
        ";
        if (!file_put_contents("{$p}/indexController.php", $con)) $msg[] = "write to controll file failed\n - {$p}/indexController";
        //create default view
        $con = '
            <h1>default view</h1>
        ...
        if (!mkdir($p, 0775, 1)) $msh[] = 'view folder create failed';
        if (!file_put_contents("{$p}/index.phtml", $con)) $msg[] = 'write to default view file failed';
        ...
        if (!mkdir($p, 0775, 1)) $msh[] = 'model folder create failed';
        if (!file_put_contents("{$p}/default.php", $con)) $msg[] = 'write to model file failed';
        ...
        if (!file_put_contents("{$p}", $con)) $msg[] = 'write to .reouer.php file failed';
        return $msg;
    }
    function upload($f) {
        // 1..........[
